...
class Machine:
    ...
    def __init__(self, input_data, account):
        ...
        self.id = input_data.get('id', None) # If id is None then is a new Instance
        self.is_new = True if self.id is None else False
        self.environment = input_data.get('environment') # based on environment we set the name (with domain)
        ...
        self.terminate_date = input_data.get('terminate_date', (datetime.now() + timedelta(days=2)).strftime('%d/%m/%Y'))
        self.terminate_time = input_data.get('terminate_time', 23)
        ...
        self.availability_zone = input_data.get('availability_zone', random.choice(account.availability_zones))
        self.profile = input_data.get('instance_profile', account.instance_profile)
        self.subnet_id = input_data.get('subnets', account.subnets.get(self.availability_zone))
    ...
    def name(self, name):
        ...
        if not self.valid_name(name):
            raise Exception(
                "{} Error: Invalid name: {}, use AlphaNumeric, - or _ and min {} chars, max {} chars".format(
                    self.__class__.__name__, name, self.name_min_len, self.name_max_len))
    ...
    def owner(self, owner):
        if not owner:
            raise Exception("{} Error: missing argument owner".format(self.__class__.__name__))
        self.__owner = owner

    @type.setter
    def type(self, type):
        if not type:
            raise Exception("{} Error: missing argument type".format(self.__class__.__name__))
        self.__type = type

    @image_id.setter
    def image_id(self, image_id):
        if not image_id:
            raise Exception("{} Error: missing argument image_id".format(self.__class__.__name__))
    ...
    def environment(self, environment):
        ...
        if not environment or environment not in environments:
            raise Exception("{} Error: environment {} is not enabled on the account settings".format(self.__class__.__name__, environment))
        self.__environment = environment

    '''
    All scheduled events use UTC time zone and the minimum precision for schedules is 1 minute.
    UTC = -2h Amsterdam - Amsterdam is 2 hours ahead of Coordinated Universal Time
    '''
    @stop_time.setter
    def stop_time(self, stop_time):
        ...
        if stop_time and (0 <= int(stop_time) <= 24):
            ...
            raise Exception("{} Error: invalid stop_time {}, valid value: 0 <= int(stop_time) <= 24".format(self.__class__.__name__, stop_time))
    ...
    def start_time(self, start_time):
        ...
        if start_time and (0 <= int(start_time) <= 24):
            ...
            raise Exception("{} Error: invalid start_time {}, valid value: 0 <= int(start_time) <= 24".format(self.__class__.__name__, start_time))

    @security_groups.setter
    def security_groups(self, security_groups):
        if not security_groups:
            security_groups = self.account.security_groups
        self.__security_groups = security_groups

    def valid_name(self, name):
        return not (self.name_min_len > len(name) or len(name) > self.name_max_len or not re.match(r'^[\w_-]+$', name))
