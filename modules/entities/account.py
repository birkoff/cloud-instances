class Account:
    def __init__(self, account_map, region):
        ...
        self.name = account_map.get("name")
    ...
    def name(self, name):
        if not name:
            raise Exception(
                "{}: Settings for account {} not found".format(
    ...
    def region(self, region):
        if not region:
            raise Exception(
                "{}: Region {} not enabled for this account".format(
                    self.__class__.__name__, region
                )
            )
    ...
    def environments(self, environments):
        if not environments:
            raise Exception(
                "{}: Missing environments from settings".format(self.__class__.__name__)
            )
    ...
    def subnets(self, subnets):
        ...
        if not subnets:
            raise Exception(
                "{} Error: Missing subnets in config".format(self.__class__.__name__)
            )
        ...
        if not self.__subnets:
            raise Exception(
                "{} Error: mapping subnets with availability_zones in region has failed".format(
    ...
    def security_groups(self, security_groups):
        if not security_groups or not isinstance(security_groups, list):
            raise Exception(
                "{} Error: Missing security_groups in config".format(
                    self.__class__.__name__
                )
            )
